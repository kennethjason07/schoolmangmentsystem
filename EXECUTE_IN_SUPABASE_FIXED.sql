-- ===================================================================
-- 🚀 CRITICAL FIXES: Execute these in your Supabase SQL Editor (FIXED VERSION)
-- ===================================================================
-- This version fixes the PostgreSQL parameter ordering issue
-- Copy and paste these commands in your Supabase SQL Editor
-- ===================================================================

-- 1. CREATE LEAVE APPLICATION OPTIMIZED FUNCTION (FIXED)
-- Parameters with defaults must come at the end
...
-- 2. CREATE LEAVE REVIEW OPTIMIZED FUNCTION (FIXED)
-- Parameters with defaults must come at the end
...
BEGIN
  ...
  -- Handle case where push_tokens table might not exist yet
  BEGIN
    SELECT COALESCE(
      json_agg(
        json_build_object(
  ...
  EXCEPTION
    WHEN undefined_table THEN
      -- push_tokens table doesn't exist yet, return empty array
      v_push_tokens := '[]'::json;
  END;
...
DROP POLICY IF EXISTS "Service role full access" ON public.push_tokens;
...
-- ✅ Fix the PostgreSQL parameter ordering issue
...
DO $$
BEGIN
  RAISE NOTICE '🎉 SUCCESS: All database functions and tables created!';
  RAISE NOTICE '📱 Push notifications table created with proper security';
  RAISE NOTICE '⚡ Leave management optimization functions ready';
  RAISE NOTICE '✅ Your system is now fully optimized!';
END $$;