...
DROP CONSTRAINT IF EXISTS unique_attendance_per_day ON student_attendance CASCADE;
DROP CONSTRAINT IF EXISTS unique_attendance_per_day ON teacher_attendance CASCADE;
...
-- ================================================================
-- STEP 4: TEST DATA CLEANUP (Optional)
-- ================================================================

-- If you want to clean up any existing duplicate data:
-- WARNING: This will delete data, use with caution!

-- Delete duplicate student attendance records (keep the latest)
-- DELETE FROM student_attendance 
-- WHERE id NOT IN (
--     SELECT DISTINCT ON (student_id, class_id, date, tenant_id) id
--     FROM student_attendance 
--     ORDER BY student_id, class_id, date, tenant_id, created_at DESC
-- );

-- Delete duplicate teacher attendance records (keep the latest)  
-- DELETE FROM teacher_attendance 
-- WHERE id NOT IN (
--     SELECT DISTINCT ON (teacher_id, date, tenant_id) id
--     FROM teacher_attendance 
--     ORDER BY teacher_id, date, tenant_id, created_at DESC
-- );
