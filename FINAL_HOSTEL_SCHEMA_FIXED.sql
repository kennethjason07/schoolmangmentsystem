-- ============================================================================
-- 🏫 FINAL HOSTEL TABLES SCHEMA - MATCHES YOUR ACTUAL DATABASE
-- ============================================================================
-- 
-- PURPOSE: Add hostel management tables to your existing school database
-- FIXED: Matches your actual roles table structure (no created_at/updated_at)
-- 
-- INSTRUCTIONS:
...
-- Step 7: Add Warden Role (FIXED - matches your actual roles table structure)
-- ============================================================================

-- Add warden role to all tenants if it doesn't exist (using your actual schema)
DO $$
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'roles') 
       AND EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'tenants') THEN
        ...
        -- Use your actual roles table structure (id, role_name, tenant_id only)
        INSERT INTO public.roles (role_name, tenant_id)
        SELECT 'Warden', t.id
        ...
        RAISE NOTICE 'Warden role added to tenants (using actual schema)';
    END IF;
END$$;
...
BEGIN
    ...
    RAISE NOTICE '✅ Warden role added (matching your schema)';
    ...
    RAISE NOTICE '• "relation public.rooms does not exist" ← SOLVED';
    RAISE NOTICE '• "relation public.beds does not exist" ← SOLVED';
    RAISE NOTICE '• hostel_fee_payments table ← ADDED';
    RAISE NOTICE '';
END$$;