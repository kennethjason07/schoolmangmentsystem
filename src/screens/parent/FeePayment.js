...
const FeePayment = ({ route }) => {
  const navigation = useNavigation();
  const { user } = useAuth();
  const { tenantId } = useTenantContext();
  const { selectedStudent } = useSelectedStudent();
  
  // Get selected student from context (SAME AS PARENTDASHBOARD)
  console.log('Parent FeePayment - SelectedStudent context:', selectedStudent);
  ...
  const fetchFeeData = async () => {
      ...
      // Get student ID from selectedStudent context (SAME AS PARENTDASHBOARD)
      const studentId = selectedStudent?.id;
      
      console.log('Parent FeePayment - Debug student ID and context:', {
      ...
      if (!user || !tenantId || !studentId) {
        console.log('Parent FeePayment - Missing required data:', {
          hasUser: !!user,
          hasTenantId: !!tenantId, 
      ...
      try {
        console.log('Parent FeePayment - Starting fetch for student:', studentId);
        ...
        const { data: studentDetails, error: studentError } = studentResult;
        
        if (schoolError) {
          console.warn('School data error (non-critical):', schoolError);
        }
        ...
        console.log('Parent FeePayment - Student details loaded:', studentDetails.name);

        // 🎯 Load fee data from student_fee_summary view (EXACT SAME AS STUDENT)
        console.log('Parent FeePayment - Loading fee data from student_fee_summary view...');
        ...
        if (feeError) {
          console.error('Parent FeePayment - Error loading fee data:', feeError);
          // Check if student has no fee data vs actual error
          if (feeError.code === 'PGRST116') {
            console.log('Parent FeePayment - No fee data found for student, showing empty state');
            setFeeStructure({
        ...
        console.log('Parent FeePayment - Fee data loaded from view:', feeData);

        // Transform fee components from JSON to array format (EXACT SAME AS STUDENT)
        ...
        console.log('Parent FeePayment - Payment history loaded:', paymentHistoryTransformed.length, 'payments');

        // Set fee structure using data from student_fee_summary view (EXACT SAME AS STUDENT)
        setFeeStructure({
        ...
        // Get real payment status data from upi_transactions table (EXACT SAME AS STUDENT)
        ...
        console.log('Parent FeePayment - Payment status loaded:', paymentStatusTransformed.length, 'transactions');
  ...
  // useEffect to call fetchFeeData when component mounts (EXACT SAME AS STUDENT)
  useEffect(() => {
    if (user) {
      fetchFeeData();
    } else {
      console.log('Parent FeePayment - No user found, waiting...');
    }
  }, [user]);
  ...
  const handlePaymentMethodSelect = (method) => {
    try {
      console.log('=== PARENT PAYMENT METHOD SELECTED ===');
      ...
      // Validate selectedStudent data
      if (!selectedStudent) {
        console.error('No student selected for payment');
        Alert.alert('Error', 'Please select a student first.');
        return;
      }

      console.log('Using selectedStudent data:', JSON.stringify(selectedStudent, null, 2));
      ...
      switch (method.id) {
        case 'UPI_QR':
          try {
            console.log('Navigating to Parent QR Payment screen');
            navigation.navigate('ParentQRPayment', {
              selectedFee,
              tenantId: tenantId
            });
            console.log('ParentQRPayment navigation successful');
          } catch (navError) {
            console.error('ParentQRPayment navigation error:', navError);
  ...
  const onRefresh = async () => {
    setRefreshing(true);
    try {
      const currentStudentId = selectedStudent?.id || user.linked_student_id;
      if (user && currentStudentId) {
        console.log('Parent FeePayment - Manual refresh triggered');
        
        // Optionally trigger fee sync to ensure consistency
        try {
          await syncFeeAfterPayment(currentStudentId, '', 0, user.tenant_id);
        } catch (syncError) {
  ...
  const handleConfirmDownload = async () => {
    ...
    try {
      ...
      console.error('❌ Parent - Isolated PDF generation error:', error);
  ...
  const handlePrintReceipt = async () => {
    ...
    try {
      ...
      console.log('✅ Parent - Isolated print completed successfully');
      setReceiptModalVisible(false);
    } catch (error) {
      console.error('❌ Parent - Isolated print error:', error);
  ...
  const generateReceiptHTML = async (receipt) => {
    ...
    try {
      console.log('📧 Parent - Generating unified receipt HTML...');
      ...
      console.log('🏫 Parent - Using unified template for Global\'s Sanmarg format');
      ...
      console.log('✅ Parent - Unified receipt HTML generated successfully');
      return htmlContent;
      ...
      console.error('❌ Parent - Error generating unified receipt:', error);
  ...
  return (
    <View style={styles.container}>
      <Header title={selectedStudent ? `${selectedStudent.name} - Fee Payment` : "Fee Payment"} showBack={true} />
