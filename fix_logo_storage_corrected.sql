...
-- 6. Drop existing policies to recreate them (safer approach)
...
-- 7. Create RLS policies for profiles bucket
...
CREATE POLICY "Users can update own files in profiles"
...
CREATE POLICY "Users can delete own files in profiles"
ON storage.objects FOR DELETE
TO authenticated
USING (bucket_id = 'profiles');

-- 8. Check what files exist in profiles bucket
SELECT 
    name,
    bucket_id,
    owner,
...
-- 9. Check school_details table for logo_url
SELECT 
    id,
    name,
    logo_url,
