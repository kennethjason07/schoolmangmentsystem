-- ====================================================
-- FACIAL RECOGNITION ATTENDANCE SYSTEM MIGRATION
-- ====================================================
...
ADD COLUMN verification_method text DEFAULT 'manual' CHECK (verification_method = ANY (ARRAY['manual'::text, 'facial_recognition'::text, 'hybrid'::text])),
ADD COLUMN recognition_event_id uuid,
ADD COLUMN recognition_confidence numeric CHECK (recognition_confidence BETWEEN 0.0 AND 1.0),
ADD COLUMN recognition_duration_ms integer,
ADD COLUMN backup_verification text, -- Manual verification if facial recognition fails
ADD COLUMN verification_notes text;

-- Add foreign key for recognition event
ALTER TABLE public.student_attendance 
...
CREATE INDEX idx_facial_templates_person ON public.facial_templates(person_id, person_type, tenant_id);
CREATE INDEX idx_facial_templates_active ON public.facial_templates(is_active, tenant_id) WHERE is_active = true;
CREATE INDEX idx_facial_templates_tenant ON public.facial_templates(tenant_id);

CREATE INDEX idx_facial_recognition_events_person ON public.facial_recognition_events(person_id, person_type, tenant_id);
CREATE INDEX idx_facial_recognition_events_performed_at ON public.facial_recognition_events(performed_at, tenant_id);
CREATE INDEX idx_facial_recognition_events_type ON public.facial_recognition_events(event_type, tenant_id);
CREATE INDEX idx_facial_recognition_events_tenant ON public.facial_recognition_events(tenant_id);

CREATE INDEX idx_student_attendance_verification ON public.student_attendance(verification_method, tenant_id);
CREATE INDEX idx_teacher_attendance_verification ON public.teacher_attendance(verification_method, tenant_id);
