...
CREATE TABLE IF NOT EXISTS stationary_purchases (
    id UUID PRIMARY KEY DEFAULT uuid_generate_v4(),
    tenant_id UUID NOT NULL REFERENCES tenants(id) ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS idx_stationary_items_tenant ON stationary_items(tenant_id);
CREATE INDEX IF NOT EXISTS idx_stationary_items_active ON stationary_items(tenant_id, is_active);
CREATE INDEX IF NOT EXISTS idx_stationary_purchases_tenant ON stationary_purchases(tenant_id);
CREATE INDEX IF NOT EXISTS idx_stationary_purchases_student ON stationary_purchases(tenant_id, student_id);
CREATE INDEX IF NOT EXISTS idx_stationary_purchases_class ON stationary_purchases(tenant_id, class_id);
CREATE INDEX IF NOT EXISTS idx_stationary_purchases_date ON stationary_purchases(tenant_id, payment_date);
CREATE INDEX IF NOT EXISTS idx_stationary_purchases_receipt ON stationary_purchases(receipt_number);

-- Function to get tenant_id for current user (if not exists)
...
-- Note: Default stationary items should be inserted through the application
-- when a user is authenticated and has proper tenant context
