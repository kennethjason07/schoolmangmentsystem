-- =====================================================
-- STUDENT DISCOUNT SYSTEM IMPLEMENTATION (CORRECTED)
-- =====================================================
...
-- Make base_amount NOT NULL after setting values (only if no nulls remain)
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM fee_structure WHERE base_amount IS NULL) THEN
        ALTER TABLE public.fee_structure ALTER COLUMN base_amount SET NOT NULL;
    END IF;
END $$;
...
DROP TRIGGER IF EXISTS student_discount_trigger ON student_discounts;
...
-- Simple policy: authenticated users can view all discounts
CREATE POLICY "Authenticated users can view discounts" ON student_discounts
    FOR SELECT 
    USING (auth.uid() IS NOT NULL);

-- Policy: Only admins can manage (insert, update, delete) discounts
...
-- 10. Add STUDENT_DISCOUNTS to your TABLES enum (reminder for code update)
-- Add to src/utils/supabase.js: STUDENT_DISCOUNTS: 'student_discounts',
...
BEGIN
    ...
    RAISE NOTICE 'Columns added to fee_structure: base_amount, discount_applied';
    ...
    RAISE NOTICE '1. Add STUDENT_DISCOUNTS to TABLES in supabase.js';
    RAISE NOTICE '2. Create discount management screens';
    RAISE NOTICE '3. Update FeeManagement screen to show discounts';
    RAISE NOTICE '========================================';
END $$;
