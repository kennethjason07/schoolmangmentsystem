-- COMPREHENSIVE MESSAGES TABLE RLS POLICIES FIX (PUBLIC SCHEMA VERSION)
...
-- Uses public schema functions to avoid auth schema permission issues

-- ==========================================
-- CREATE REQUIRED FUNCTIONS IN PUBLIC SCHEMA
-- ==========================================
...
-- Create a function to get current user's tenant_id (in public schema)
...
WITH CHECK (
  ...
  EXISTS (
    ...
    AND u.tenant_id = public.current_user_tenant_id()
  )
);
...
SELECT 'Testing current_user_tenant_id function...' as test_status;
SELECT public.current_user_tenant_id() as current_tenant_id;
...
SELECT '
🔐 MESSAGES TABLE RLS POLICIES SUCCESSFULLY IMPLEMENTED! (PUBLIC SCHEMA VERSION)

FUNCTIONS CREATED:
✅ public.get_user_tenant_id() - Get tenant ID for any user
✅ public.current_user_tenant_id() - Get current user tenant ID
