...
DROP POLICY IF EXISTS "Enable all for authenticated users with matching tenant" ON public.stationary_items;
DROP POLICY IF EXISTS "Enable all for authenticated users with matching tenant" ON public.stationary_purchases;
DROP POLICY IF EXISTS "Enable all for authenticated users with matching tenant" ON public.stationary_stock_adjustments;

-- Drop existing triggers if they exist (to avoid conflicts)
DROP TRIGGER IF EXISTS trigger_update_stock_after_purchase ON public.stationary_purchases;
DROP TRIGGER IF EXISTS trigger_update_stock_after_adjustment ON public.stationary_stock_adjustments;
...
-- Grant necessary permissions (optional but recommended)
GRANT ALL ON public.stationary_items TO authenticated;
GRANT ALL ON public.stationary_purchases TO authenticated;
GRANT ALL ON public.stationary_stock_adjustments TO authenticated;

-- Insert some default stationary items (optional - uncomment and replace 'your-tenant-id' with actual tenant ID)
...
-- After running this script, the stationary management system should work without RLS policy errors.
